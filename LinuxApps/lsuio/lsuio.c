...
static void show_device(struct uio_info_t *info)
{
	...
	printf("%s: name=%s, version=%s, events=%d\n",
	       dev_name, info->name, info->version, (unsigned)info->event_count);
}
...
static int show_map(struct uio_info_t *info, int map_num)
{
	...
	printf("\tmap[%d]: addr=0x%08X, size=%d",
	       map_num,
	       (unsigned)info->maps[map_num].addr,
	       (unsigned)info->maps[map_num].size);
...
static int decode_switches (int argc, char **argv)
{
	...
	while (1) {
		opt = getopt_long(argc,argv,"hmf:u:v",long_options,&opt_index);
		if (opt == EOF)
			break;
		switch (opt) {
			case 'm' : opt_mmap = 1;
				   break;
			case 'u' : uio_filter = atoi(optarg);
				   break;
			case 'f' : uio_name = optarg;
				   break;
...
Options:\n\
  ...
  -u, --uiodev     display device \"u\" attributes\n\
  -f, --find       display device using name attribute\n\
